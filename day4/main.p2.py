...
def get_complete(boards, draws):
    masks = np.isin(boards, draws)
...
def run(lines):
    ...
    for i in range(len(draws) - 1, 0, -1):
        complete, _ = get_complete(boards, draws[:i])
        if not complete.all():
            # Backtrack 1 step
            _, masks = get_complete(boards, draws[: i + 1])
            board = boards[~complete, :, :]
            mask = masks[~complete, :, :]
            return int(board[~mask].sum() * draws[i])
